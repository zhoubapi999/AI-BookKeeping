...
<template>
  <Drawer v-model:open="isOpen">
    <DrawerContent>
      <DrawerHeader>
        <DrawerTitle>{{ editingId ? '编辑分类' : '新建分类' }}</DrawerTitle>
      </DrawerHeader>
      <div class="p-4 max-h-[75vh] overflow-y-auto space-y-4">
        ...
        <div class="gap-4 grid grid-cols-2">
          ...
          <div class="space-y-2">
            ...
            <div class="flex gap-2 h-10 items-center">
              <Input v-model="formData.color" type="color" class="p-1 h-full w-full" />
            </div>
          </div>
        </div>
        ...
        <div class="space-y-2">
          ...
          <div class="gap-2 grid grid-cols-6">
            <div
              ...
              :class="formData.icon === icon.value ? 'bg-primary/10 border-primary text-primary' : 'border-transparent hover:bg-gray-100 text-gray-500'"
        ...
        <div class="mt-6">
          <h3 class="text-sm text-gray-600 font-medium mb-3">
            管理分类
          </h3>
          <div class="gap-2 grid grid-cols-1">
            ...
            >
              ...
              <div class="flex gap-2">
                <Button variant="ghost" size="icon" class="h-8 w-8" @click="handleEdit(c)">
                  <Edit class="h-4 w-4" />
                </Button>
                <Button variant="ghost" size="icon" class="text-red-500 h-8 w-8" @click="handleDelete(c.id)">
